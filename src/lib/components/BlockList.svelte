		...
		<form on:submit|preventDefault={makeNewRule} class=" text-black">
			<div class="grid grid-cols-10">
				...
				<div class="text-white grid grid-cols-7 col-span-2" id="days">
					<span
						class="day-select"
						on:click={() => updateNewRuleDay(0)}
						class:day-selected={newRule.days[0] !== 0}>S</span
					>
					<span
						class="day-select"
						on:click={() => updateNewRuleDay(1)}
						class:day-selected={newRule.days[1] !== 0}>M</span
					>
					<span
						class="day-select"
						on:click={() => updateNewRuleDay(2)}
						class:day-selected={newRule.days[2] !== 0}>T</span
					>
					<span
						class="day-select"
						on:click={() => updateNewRuleDay(3)}
						class:day-selected={newRule.days[3] !== 0}>W</span
					>
					<span
						class="day-select"
						on:click={() => updateNewRuleDay(4)}
						class:day-selected={newRule.days[4] !== 0}>T</span
					>
					<span
						class="day-select"
						on:click={() => updateNewRuleDay(5)}
						class:day-selected={newRule.days[5] !== 0}>F</span
					>
					<span
						class="day-select"
						on:click={() => updateNewRuleDay(6)}
						class:day-selected={newRule.days[6] !== 0}>S</span
					>
				</div>
